...
def user_audio_confirmation(path):
    ...
    confirmation = input("Have you checked the audio and do you want to proceed with posting a training? (y/n): ")
    return confirmation.lower() == "y"
...
def main():
    parser = argparse.ArgumentParser(description='Download a video from YouTube and extract frames or audio')
    ...
    parser.add_argument('--remove_blur', help='remove blurry frames', default=True, action='store_true')
    ...
    if video_url.startswith("http"):
        video_file_path = download_youtube_video(video_url, save_path=download_directory, audio_only=args.audio)
    else:
